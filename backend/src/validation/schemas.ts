 ...
 * Each schema lives here so it can be reused across routes and tested independently.
 */
...
const objectId = () =>
  z
    ...
    .refine((v) => mongoose.isValidObjectId(v), { message: 'Invalid ObjectId format' });

/** HH:MM 24-hour time */
const timeHHMM = () =>
  z
    .string()
    .regex(/^([01]\d|2[0-3]):[0-5]\d$/, 'Time must be in HH:MM 24-hour format (e.g. 09:30)');
...
const dateYYYYMMDD = () =>
  z
    ...
    .refine((v) => !isNaN(Date.parse(v)), { message: 'Date is not a real calendar date' });
...
const breakPeriodSchema = z.object({
  ...
  { message: 'Break start time must be before end time', path: ['endTime'] }
);
...
const workingHoursSchema = z.object({
  ...
  { message: 'Working start time must be before end time', path: ['endTime'] }
);
...
export const createDoctorSchema = z.object({
  ...
  slotDuration: z
    ...
    .default(15),
  workingHours: workingHoursSchema.default({ startTime: '09:00', endTime: '17:00' }),
...
export const createPatientSchema = z.object({
  ...
  mobile: z
    ...
    .trim()
    .regex(/^[0-9]{10,15}$/, 'Mobile must be 10–15 digits with no spaces or symbols'),
  age: z
...
// ── Appointment filter query params ───────────────────────────────────────────

export const appointmentQuerySchema = z.object({
  doctorId: z.string().optional().refine(
    (v) => !v || mongoose.isValidObjectId(v),
    { message: 'doctorId must be a valid ObjectId' }
  ),
  patientId: z.string().optional().refine(
    (v) => !v || mongoose.isValidObjectId(v),
  ...
  status: z.enum(['booked', 'arrived', 'completed', 'cancelled', 'no_show']).optional(),
