...
const AppLayout = ({ children }) => {
  const { user, logout, isAdmin, isDoctor, isReceptionist } = useAuth();
  ...
  const getRoleBadge = () => {
    if (isAdmin) return { label: 'Admin', color: '#e53935' };
    if (isDoctor) return { label: 'Doctor', color: '#1976d2' };
    if (isReceptionist) return { label: 'Reception', color: '#388e3c' };
      ...
      <aside style={{ ...styles.sidebar, width: sidebarOpen ? 240 : 64 }}>
        <div style={styles.sidebarHeader}>
          {sidebarOpen && (
            <div style={styles.logo}>
              ...
              <span style={styles.logoText}>MedEMR</span>
            </div>
          )}
          <button
        ...
        <nav style={styles.nav}>
          {items.map((item) => (
            <NavLink
              key={item.path}
              to={item.path}
              end={item.path.endsWith('/') || item.path.split('/').length <= 2}
              style={({ isActive }) => ({
                ...styles.navLink,
        ...
        <div style={styles.sidebarFooter}>
          <div style={styles.userInfo}>
            <div style={{ ...styles.roleBadge, background: badge.color }}>
              {badge.label}
            </div>
            {sidebarOpen && (
