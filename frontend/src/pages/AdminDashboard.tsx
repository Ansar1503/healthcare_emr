...
const AdminDashboard = () => {
  ...
  const [rf, setRf] =
    useState<CreateReceptionistFormValues>(EMPTY_RECEPTIONIST);
  const [rfErrors, setRfErrors] = useState<
    Partial<CreateReceptionistFormValues>
  >({});
  ...
  const { data: usersRaw, execute: refetchUsers } = useApi<IUser[]>(
    () => userService.getAll({ role: "receptionist" }),
    [],
    true,
  );
          ...
          <section>
            ...
            {showDoctorForm && (
              <div style={s.formCard}>
                ...
                <form
                  onSubmit={(e) => void handleCreateDoctor(e)}
                  style={s.formGrid}
                  noValidate
                >
                  <Field label="Full Name *" id="d-name" error={dfErrors.name}>
                    <input
                      id="d-name"
                      style={fieldStyle(dfErrors.name)}
                      value={df.name}
                  ...
                  <Field
                    label="Department *"
                    id="d-dept"
                    error={dfErrors.department}
                  >
                    <select
                      id="d-dept"
                      style={fieldStyle(dfErrors.department)}
                      value={df.department}
                      onChange={(e) =>
                        setDf({ ...df, department: e.target.value })
                      }
                    >
                  ...
                  <Field label="Specialization" id="d-spec">
                    <input
                      id="d-spec"
                      style={s.input}
                      value={df.specialization ?? ""}
                      onChange={(e) =>
                        setDf({ ...df, specialization: e.target.value })
                      }
                    />
                  </Field>
                  <Field
                    ...
                    id="d-slot"
                    error={String(dfErrors.slotDuration ?? "")}
                  >
                    <select
                      id="d-slot"
                      style={s.input}
                      value={df.slotDuration}
                      onChange={(e) =>
                        setDf({ ...df, slotDuration: Number(e.target.value) })
                      }
                    >
                  ...
                  <Field
                    label="Login Email *"
                    id="d-email"
                    error={dfErrors.email}
                  >
                    <input
                      id="d-email"
                      type="email"
                      style={fieldStyle(dfErrors.email)}
                      value={df.email}
                  ...
                  <Field
                    label="Password *"
                    id="d-pass"
                    error={dfErrors.password}
                  >
                    <input
                      id="d-pass"
                      type="password"
                      style={fieldStyle(dfErrors.password)}
                      value={df.password}
                      onChange={(e) =>
              ...
              {doctors.map((d) => (
                <div key={d._id} style={s.tableRow}>
                  <div>
                    ...
                    {d.specialization && (
                      <span style={s.meta}> · {d.specialization}</span>
                    )}
                  </div>
          ...
          <section>
            ...
            {showReceptionForm && (
              <div style={s.formCard}>
                ...
                <form
                  onSubmit={(e) => void handleCreateReception(e)}
                  style={s.formGrid}
                  noValidate
                >
                  <Field label="Full Name *" id="r-name" error={rfErrors.name}>
                    <input
                      id="r-name"
                      style={fieldStyle(rfErrors.name)}
                      value={rf.name}
                  ...
                  <Field label="Email *" id="r-email" error={rfErrors.email}>
                    <input
                      id="r-email"
                      type="email"
                      style={fieldStyle(rfErrors.email)}
                      value={rf.email}
                  ...
                  <Field
                    label="Password *"
                    id="r-pass"
                    error={rfErrors.password}
                  >
                    <input
                      id="r-pass"
                      type="password"
                      style={fieldStyle(rfErrors.password)}
                      value={rf.password}
                      onChange={(e) =>
                        setRf({ ...rf, password: e.target.value })
                      }
                    />
                  </Field>
                  <div
            ...
            <div style={s.table}>
              {receptionists.map((u) => (
                <div key={u.id} style={s.tableRow}>
                  ...
                  <div style={s.rowRight}>
                    ...
                    <button
                      style={s.btnXs}
                      onClick={() => void handleToggleActive(u.id)}
                    >
              ...
              {receptionists.length === 0 && (
                <p style={s.empty}>No receptionists yet.</p>
              )}
            </div>
          </section>
        )}
